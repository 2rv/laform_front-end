...
export async function getPostalCode(value: string) {
  const response = await httpRequest({
...
export function getCityCodeByKladr(kladr: string) {
  return async (dispatch: any) => {
    dispatch({
      type: FIND_ADRESS_ACTION_TYPE.GET_CITY_CODE_PENDING,
    });
    try {
      const response = await httpRequest({
        url: GET_CITY_CODE.URL + kladr,
        method: GET_CITY_CODE.TYPE,
      });

      dispatch({
        type: FIND_ADRESS_ACTION_TYPE.GET_CITY_CODE_SUCCESS,
        data: response.data,
      });
    } catch (error: any) {
      dispatch({
        type: FIND_ADRESS_ACTION_TYPE.GET_CITY_CODE_ERROR,
